...
class OpenDoorServer(Node):
    def __init__(self):
        ...
        self._action_server = ActionServer(
            ...
            callback_group=self._cb_group,
            goal_callback=lambda _r: GoalResponse.REJECT if self._motion_lock.locked() else GoalResponse.ACCEPT,
    ...
    def _execute(self, goal_handle):
        ...
        # One motion at a time: refuse if the action or a debug service holds the lock.
        if not self._motion_lock.acquire(blocking=False):
            goal_handle.abort()
            result.success = False
            result.message = "busy: another open_door motion is already running"
            return result
        try:
