...
#define ROS_PARAM_LIST                                                                                                 \
  ...
  LIST_ENTRY(subordinate_delay_off_master_usec,                                                                        \
              ...
              int, 0)\
  LIST_ENTRY(color_white_balance, "Manual white balance K value (-1 for Auto)", int, -1) \
  LIST_ENTRY(color_exposure_time, "Manual exposure time in microseconds (-1 for Auto)", int, -1) \
  LIST_ENTRY(color_gain, "Manual gain value", int, 128) \
  LIST_ENTRY(color_powerline_frequency, "Anti-flicker (1:50Hz, 2:60Hz)", int, 2) \
  LIST_ENTRY(color_backlight_compensation, "Backlight compensation (0:Off, 1:On)", int, 0) \
  LIST_ENTRY(color_brightness, "Brightness (0-255)", int, 128) \
  LIST_ENTRY(color_contrast, "Contrast (0-10)", int, 5) \
  LIST_ENTRY(color_saturation, "Saturation (0-63)", int, 32) \
  LIST_ENTRY(color_sharpness, "Sharpness (0-4)", int, 2)
