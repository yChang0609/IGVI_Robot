    ...
    imu_stream_end_of_file_(false)
{
  ...
  this->declare_parameter("color_white_balance", rclcpp::ParameterValue(-1));
  this->declare_parameter("color_exposure_time", rclcpp::ParameterValue(-1));
  this->declare_parameter("color_gain", rclcpp::ParameterValue(128));
  this->declare_parameter("color_brightness", rclcpp::ParameterValue(128));
  this->declare_parameter("color_contrast", rclcpp::ParameterValue(5));
  this->declare_parameter("color_saturation", rclcpp::ParameterValue(32));
  this->declare_parameter("color_sharpness", rclcpp::ParameterValue(2));
  this->declare_parameter("color_backlight_compensation", rclcpp::ParameterValue(0));
  this->declare_parameter("color_powerline_frequency", rclcpp::ParameterValue(2));
...
k4a_result_t K4AROSDevice::startCameras()
{
  ...
  if (k4a_device_)
  {
    ...
    try {
        // 1. 白平衡控制 (支援 -1 自動切換)
        if (params_.color_white_balance == -1) {
            k4a_device_.set_color_control(K4A_COLOR_CONTROL_WHITEBALANCE, K4A_COLOR_CONTROL_MODE_AUTO, 0);
        } else {
            k4a_device_.set_color_control(K4A_COLOR_CONTROL_WHITEBALANCE, K4A_COLOR_CONTROL_MODE_MANUAL, params_.color_white_balance);
        }

        // 2. 曝光控制 (支援 -1 自動切換)
        if (params_.color_exposure_time == -1) {
            k4a_device_.set_color_control(K4A_COLOR_CONTROL_EXPOSURE_TIME_ABSOLUTE, K4A_COLOR_CONTROL_MODE_AUTO, 0);
        } else {
            k4a_device_.set_color_control(K4A_COLOR_CONTROL_EXPOSURE_TIME_ABSOLUTE, K4A_COLOR_CONTROL_MODE_MANUAL, params_.color_exposure_time);
        }

        // 3. 其他僅支援手動模式的參數
        k4a_device_.set_color_control(K4A_COLOR_CONTROL_GAIN, K4A_COLOR_CONTROL_MODE_MANUAL, params_.color_gain);
        k4a_device_.set_color_control(K4A_COLOR_CONTROL_BRIGHTNESS, K4A_COLOR_CONTROL_MODE_MANUAL, params_.color_brightness);
        k4a_device_.set_color_control(K4A_COLOR_CONTROL_CONTRAST, K4A_COLOR_CONTROL_MODE_MANUAL, params_.color_contrast);
        k4a_device_.set_color_control(K4A_COLOR_CONTROL_SATURATION, K4A_COLOR_CONTROL_MODE_MANUAL, params_.color_saturation);
        k4a_device_.set_color_control(K4A_COLOR_CONTROL_SHARPNESS, K4A_COLOR_CONTROL_MODE_MANUAL, params_.color_sharpness);
        k4a_device_.set_color_control(K4A_COLOR_CONTROL_BACKLIGHT_COMPENSATION, K4A_COLOR_CONTROL_MODE_MANUAL, params_.color_backlight_compensation);
        k4a_device_.set_color_control(K4A_COLOR_CONTROL_POWERLINE_FREQUENCY, K4A_COLOR_CONTROL_MODE_MANUAL, params_.color_powerline_frequency);

        RCLCPP_INFO(this->get_logger(), "Applied camera hardware controls with Auto(-1) support.");
    } catch (const std::exception& e) {
        RCLCPP_WARN_STREAM(this->get_logger(), "Failed to set color controls: " << e.what());
    }
  }
