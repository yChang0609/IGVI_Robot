...
def generate_launch_description():
    ...
    kinect_node = Node(
        ...
        parameters=[{
            ...
            'color_exposure_time': int(kinect.get('exposure_time_absolute', -1)),
            'color_gain': int(kinect.get('gain', -1)),
            'color_white_balance': int(kinect.get('white_balance', -1)),
            'color_brightness': int(kinect.get('brightness', 128)),
            'color_contrast': int(kinect.get('contrast', 5)),
            'color_saturation': int(kinect.get('saturation', 32)),
            'color_sharpness': int(kinect.get('sharpness', 2)),
            'color_backlight_compensation': int(1 if kinect.get('backlight_compensation', False) else 0),
            'color_powerline_frequency': int(2 if kinect.get('powerline_frequency', 60) == 60 else 1),
        }],
    )
